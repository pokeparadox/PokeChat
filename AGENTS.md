...
- **Phase 26:** Chat Session Logging ✅ (per-session log files for debugging abnormal responses; basic/verbose modes; log rotation; see `.agents/history.md`)
- **Phase 27:** Built-in Tool Layer 📝 (lightweight tool system invoked directly from response rules via `{tool:name}` markers; `ITool` interface, `ToolRegistry`, seed tools for web search and URL fetching; JSON config; graceful degradation; see `.plans/phase27-built-in-tools.md`)
