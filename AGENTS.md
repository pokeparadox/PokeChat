...
- **Phase 17:** Temporal Knowledge ✅ (TemporalExpression entity + 15 seeded time expressions, FactEntity.TimeContext/MentionedAt columns, ExtractTimeContext/GetFactsByTimeRange/GetFactsWithTimeContext in KnowledgeStore, time context extraction in ChatSession, temporal query handling in ResponseEngine, 7 new tests)
- **Phase 18:** Inference / Simple Reasoning ✅ (Category chain via is_a WordLinks, contradiction detection for like↔hate, generalisation inference with 50% display chance, 5 new KnowledgeStore methods, 6 inference response categories, 12 new tests)
- **Phase 19:** Session Summarisation ✅ (SessionId on conversations, ConversationSession entity, summary build via fact lookup, exit recap, 9 new tests)
- **Phase 20:** Self-Learning Response Patterns ✅ (learned_response_rules + response_feedback tables, correction detection in ChatSession, confidence-based rule selection in ResponseRules, duplicate check via Local+DB, fix pre-existing bug in "not what I meant" feedback pattern not matching)
- **Phase 21:** Multi-Turn Topic Tracking ✅ (topic stack across 5 turns, topic reference after follow-up exhaustion, 11 new tests)
...
- **Phase 29b:** Data-Driven MCP Tool Triggers ✅ (tool triggers defined in `mcp.json`; zero-code MCP tool addition; auto-generated catch-all triggers; POS auto-seeding; removes hardcoded tool rules; `.plans/phase29b-mcp-tool-triggers.md` completed)
- **Phase 30:** Enhanced LLM Integration ✅ (AlwaysOn mode removes offer/cap; 22 category LLM enhancement via prompt map; MCP tool result summarisation; dictionary LLM fallback + save confirmation; contradiction/generalisation LLM inference; LLM story generation; correction LLM reflection; 13 new tests, 327/327 pass)
- **Phase 31:** Clarification/Classification Cancel ✅ (user can say "typo/never mind/mistake/my bad/forget it" during unknown word or classification flow; bot cancels instead of learning the word. Removes from POS dict + spell checker. New: `IsClarificationCancelled`, `RemoveLearnedWord`, `RemoveFromDictionary`. 3 new tests, 330/330 pass)
- **Phase 32:** End-of-Session LLM Homework Check ✅ (LLM reviews conversation after exit; `GenerateHomeworkCheck` bypasses MaxCallsPerSession; `DeactivateLearnedRule` in KnowledgeStore; 3 bot response categories; JSON parsing with `SnakeCaseLower`. 12 new tests, 346/347 pass)
- **Phase 33:** Word Game UX Improvements ✅ (hides mid-game story reveal; thinking indicator during LLM calls; `ApplyGameGrammarFilter` 8-step pipeline; optional LLM story summary at game end via `GenerateGameStorySummary`; updated DB seed categories. 12 new/updated tests, 355/356 pass)
- **Phase 34:** Dad Jokes + Riddles ✅ (setup→punchline flow for dad jokes; multi-turn riddles with hints, 3 attempts, give-up. `Joke`/`Riddle` entities, schema, DbContext, ContextKeys, KnowledgeStore, DbSeeder, ChatSession routing + handlers. 12 new tests, 384/385 pass)
- **Phase 35:** Poetry Generation (Haiku & Limerick) ✅ (SyllableCounter, RhymeMatcher, PoetryGenerator, 12 haiku + 10 limerick templates, 130+ rhyme group entries, 12 new tests, 484/484 pass)
...
- **Clarification cancel:** `HandleClarification` and `HandleClassification` in ChatSession check `IsClarificationCancelled()` before learning any word. Detection phrases: `typo`, `never mind`, `forget it`, `my bad`, `that was a mistake`, `it was a mistake`, `my mistake`, `dont worry about it`, `nothing`, `ignore it`, `scratch that`, `strike that`. Cancellation clears pending state without storing the word; classification cancellation also removes the word from the POS dictionary and spell checker via `RemoveLearnedWord`/`RemoveFromDictionary`.
...
- **Inference (Phase 18):** `DetectContradiction` finds existing facts with same subject + same object + opposite verb (like↔hate, love↔dislike). Only runs for Preference/Dislike predicates. Contradiction detection blocks fact storage and sets `LastContradiction` context key. Generalisation inference runs after contradiction check, sets `InferredGeneralisation` context key, displayed at 50% chance. `HandleInferenceResponse` in ResponseEngine fires before rule matching to catch contradictions. `GetCategoryChain` uses BFS with `visited` HashSet to prevent cycles. Inference seed data uses `is_a` link type in WordLinks table.
- **Response Rules (Phase 20):** `ResponseRules.MatchRule` merges seeded rules (`response_rules` + `response_rule_responses`) with `learned_response_rules` (loaded from DB, ordered by confidence descending). Learned rules with confidence >= 7 are preferred over seed rules (confidence 8). Learned rules start at confidence 5; each successful match +1 (cap 10); each negative feedback -2 (floor 1, deactivates at 0). `ResponseRuleRecord` has `RuleId`, `IsLearned`, `Confidence`.
- **Correction Detection (Phase 20):** `ChatSession.TryHandleCorrection` runs after rename check, before sentiment analysis. Detects `you should say X`, `say X instead`, `try saying X`, `when/if I say X you should/could Y` via regex on original input with `IgnoreCase`. Learns new response pattern by extracting the last word from the previous input as a `\bword\b` regex pattern. Negative feedback ("that's not right" etc.) records negative feedback on last rule used. Positive feedback ("that's better" etc.) records positive feedback. `LastRuleId` and `LastRuleIsLearned` context keys set by ResponseEngine after rule matching.
- **Learned rules storage (Phase 20):** `learned_response_rules` table (Pattern, ResponseTemplate, InputType, LearnedFromUserId FK→users, Confidence 1-10 default 5, IsActive default true, CreatedAt). `response_feedback` table (RuleId, IsLearnedRule discriminator, UserId FK→users, Feedback string, CorrectionText, CreatedAt). `KnowledgeStore.LearnResponseRule` checks Local then DB for duplicates. `GetLearnedRules` returns only active rules ordered by Confidence desc. `AdjustConfidence` clamps 1-10, sets IsActive=false at 1. `RecordFeedback` stores feedback and adjusts confidence.
- **Garbage triple filter (Post-Phase 21):** `ChatSession.FunctionWords` (`"not"`, `"never"`, `"no"`) filters triples in `ProcessSentence` where `predicateType == PredicateType.General` and `resolvedObject` is a single function word. Prevents multi-verb sentences like "I think you don't know" from producing garbage triples like (you, do, not) which would generate nonsensical context follow-ups.
- **"ok" in POS dictionary (Post-Phase 21):** `"ok"` was missing from `pos_dictionary.json` (`"okay"` was present at line 4411). Levenshtein suggested `"of"` (distance 1). Fixed by adding `{"Word": "ok", "Type": "adjective"}` to the JSON after `"okay"`.
- **Sentiment follow-up acknowledgement (Post-Phase 21):** After `emotion_followup` asked "Are you feeling better now?", the answer was ignored — `HandleSentiment()` skipped mild emotions (intensity < 2), falling through to context follow-up. Fixed via `PendingSentimentFollowUp` context key — set in `HandleSentiment` when `emotion_followup` fires, checked in `GenerateResponse` before sentiment analysis. If set + intensity ≥ 1, returns sentiment-aware acknowledgement (positive/negative/fallback templates). Cleared after acknowledgement.
- **Context follow-up self templates:** When the bot's context follow-up subject matches the user's name, it uses `context_followup_self` ("Tell me about yourself, {0}.") and `context_followup_with_object_self` ("Tell me more about your {1}.") instead of the generic templates that would reference the user in third person. `ResponseEngine.SetCurrentUserName()` tracks the current user name for this detection. Falls back to generic templates if the self category is missing from DB.
- **Pronoun resolution "they"/"their":** `ContextTracker.ResolvePronoun` now resolves `"they"/"their"` to `LastObject` first (then `LastSubject`), matching `"them"` behavior. Previously resolved only to `LastSubject`, causing "They are spotty" → (user, are, spotty) instead of (frogs, are, spotty) when frogs was the LastObject.
- **Word Classification (Phase 25):** After the user teaches a new word (via `HandleClarification` non-suggestion path), the bot asks one classification question: "Is it a person, place, thing, or verb?" The response is parsed via keyword matching (`ParseWordType`), and the word's `WordType` is updated in `pos_dictionary` (noun/verb/adjective). For noun subcategories (person/place/thing), also stored in `noun_categories` table. If classified as "place", one extra follow-up asks "Have you ever been to {word}?" and stores a visit fact if affirmed. Classification is limited to first 2 words per session via `PendingClassificationCount` context key. Changes are saved immediately via `_knowledgeStore.Save()` in each state handler.
...
- **Tool marker `{$N}` substitution:** Seeded response rules like `{tool:web_search}` couldn't pass the search query to the tool. Added `{$N}` regex group substitution in `ResponseEngine.GenerateResponse` — templates can now use `{tool:web_search:{$2}}` to reference captured groups. Requires manual DB update for existing databases since the seeder only runs on empty DB (use `UPDATE ResponseRuleResponses SET ResponseText = REPLACE(ResponseText, '{tool:web_search}', '{tool:web_search:{$2}}')`).
- **Interview mode rewrite (bot-driven, noun-based questions):** Interview mode was reversed — now the **bot asks questions** about known nouns (from `noun_categories` + `pos_dictionary` where word_type="noun"), and the user (non-LLM) or LLM (LLM mode) answers. `IInterviewEngine` interface replaced `GenerateUserInput`/`AddExchange(string,string)` with `GenerateQuestion`/`GenerateAnswer(string)`/`AddExchange(string,string,string)`. NonLlmInterviewEngine and InterviewEngine both use category-aware question templates (person/place/thing). LLM version builds conversation history from exchanges for context-aware answers. Stop mechanism: `Console.KeyAvailable` replaced with `Console.ReadLine` prompt between turns; `quit`/`exit`/`q` added to `InterviewStopPhrases`. `InterviewSystemPrompt` updated to answer-focused persona. ChatSession flow: bot asks question → get answer (LLM auto or user via "You:" prompt → ProcessInput → response → record exchange. 3 new tests.
- **Interview mode facts stored under wrong user:** During interview, `_currentUserId` was set to the Interviewer identity (ID 2). `ProcessInput` stored learned facts under Interviewer's user ID, making them invisible to the original user after `EndInterviewMode()` restored the original ID. Fixed by temporarily swapping `_currentUserId` to `_savedUserId` before `ProcessInput` during interview mode. LLM `InterviewSystemPrompt` also updated to emphasize simple single-sentence SVO-structured answers for better NLP parsing.
